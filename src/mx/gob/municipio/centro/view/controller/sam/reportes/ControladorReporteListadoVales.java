...
public class ControladorReporteListadoVales extends ControladorBase {
	...
	public ModelAndView  requestGetControlador( Map modelo, HttpServletRequest request) {
		...
		modelo.put("rs",new JRMapCollectionDataSource(this.getListadoVales(unidad, estatus,cve_benefi,numvale,numcontrato, fechaIni,fechaFin,this.getSesion().getEjercicio(),tipoGasto,this.getSesion().getIdUsuario(),verUnidad, privilegio)));	
		
		return new ModelAndView("rpt_listado_vales",modelo);
	}
	
	public List <Map<String, Object>>getListadoVales(String unidad, String  estatus ,String clv_benefi , String numvale, String numcontrato, String fechaInicial, String fechaFinal , Integer ejercicio, String tipoGasto, Integer idUsuario, String verUnidad, Boolean privilegio){
		return this.gatewayVales.getListaDeValesPorEjemplo(unidad, estatus ,clv_benefi, numvale,numcontrato, this.formatoFecha(fechaInicial), this.formatoFecha(fechaFinal) , ejercicio, tipoGasto, idUsuario, verUnidad, privilegio);
	}
