...
public class ControladorReporteOrdenPago extends ControladorBase {
	...
	public ModelAndView  requestGetControlador( Map modelo, @RequestParam("cve_op")  Long  idOrden ) {	
		...
		//modelo.put("ANO_MUNICIPIO", this.getJdbcTemplate().queryForObject("SELECT PRE_FIJO_CLAVE_PRESU FROM SAM_CONFIGURACION_ADICIONAL ",Integer.class));
		...
		List <Map> comp = this.getJdbcTemplate().queryForList("SELECT DISTINCT NUM_VALE FROM COMP_VALES INNER JOIN SAM_VALES_EX ON (SAM_VALES_EX.CVE_VALE = COMP_VALES.CVE_VALE) WHERE CVE_OP = ? AND COMP_VALES.TIPO NOT IN ('CF','FA')", new Object[]{idOrden});
		String temp = "";
		for (Map val: comp){
	...
	private String tieneVale(Long idOrden) {
		...
		int vales= this.getJdbcTemplate().queryForInt("select  count(*) from COMP_VALES a , SAM_VALES_EX  b   where  a.CVE_VALE=b.CVE_VALE and a.TIPO NOT IN ('CF','FA') AND  a.cve_op=? ",new Object []{idOrden});
		if (vales > 0)
			tieneVales="SI";
		return tieneVales;		
	}
