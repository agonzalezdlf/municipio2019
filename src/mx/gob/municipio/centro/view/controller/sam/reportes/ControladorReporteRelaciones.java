...
public class ControladorReporteRelaciones extends ControladorBase {
	...
	public ModelAndView  requestGetControlador( Map modelo, @RequestParam("id_relacion")  Long  idRelacion ) {
		...
		Integer idGrupo = this.getJdbcTemplate().queryForInt("SELECT * FROM SAM_GRUPO_CONFIG_USUARIO GCP WHERE EXISTS (SELECT * FROM SAM_PERSONAS WHERE CVE_PERS=GCP.ID_USUARIO)	AND GCP.ID_USUARIO=? AND ASIGNADO=1", new Object[]{modelo.get("CVE_PERS").toString()}); //();
				
		gatewayFirmasDocumentos.getFirmasDocumentos(idGrupo, modelo);
		List <Map<String, Object>> lst =  new ArrayList();       
		List <Map<String, Object>> lstdetalles = gatewayOrdenDePagos.cargarRelacionesDocumentos(idRelacion);
		String unidad = "";
		for(Map<String, Object> row: lstdetalles){
