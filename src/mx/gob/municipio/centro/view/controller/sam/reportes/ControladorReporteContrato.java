...
public class ControladorReporteContrato extends ControladorBase {
	...
	public ModelAndView  requestGetControlador( Map modelo, @RequestParam("cve_contrato")  Long  cve_contrato ) {		
		...
		List <Map<String, Object>> lstcontratos =getJdbcTemplate().queryForList("SELECT DESCRIPCION DESCRIPTIONS FROM SAM_CONTRATOS WHERE CVE_CONTRATO=?", new Object[]{cve_contrato});
		String detalle_contrato =null;
		for(Map fila: lstcontratos){
			
			detalle_contrato = (String) fila.get("DESCRIPTIONS");
		}
		
		//detalle_contrato= (String) modelo.get("CONCEPTO");
		modelo.put("CONCEPTO", detalle_contrato);
		
		List <Map<String, Object>> detalles = this.gatewayContratos.getConceptosContratoAgrupados(cve_contrato);
		...
		modelo.put("IMPORTE_LETRAS", cantidadLetra.substring(1, cantidadLetra.length()-1));
