...
//-------------------------- Agregaba el movimiento del vale en la orden de pago -------------------------------------- 	
public void insertaVale(Integer idCons,Integer vale,double importe,Long idOrden, String tipo, Date fecha, Date fechaDeposito, int ejercicio, int cve_pers, int proyecto, String clv_partid ){
	try{
		//obtener el vale 
		idCons = this.getJdbcTemplate().queryForInt("SELECT MAX(CONS_VALE)+1 FROM CONCEP_VALE WHERE CVE_VALE = ?", new Object[]{vale});
...
//-------------------------- Actualizaba el movimiento del vale en la orden de pago -------------------------------------- 
...
//-------------------------- Eliminaba el movimiento del vale en la orden de pago -------------------------------------- 
...
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------- Agrega la Comprobacion del Vale por reembolso a finanzas ------------------------------------------------------------*/

	/*Carga el importe restante en la vista del reembolso liquido por finanzas*/
	...
	/*Muestra en la tabla la lista de los movimentos del vale en la vista*/
	...
	/*Carga el combo del reemboloso liquido*/
...
public  String actualizarConceptoPrincipalVale(Integer idVale,Integer vale,double importe,double importeValeAnte,Long idOrden, int idproyecto, String partida, String tipo,  Date fecha, Date fechaDeposito, int ejercicio, int cve_pers){
	...
	/*------------  GUARDA EN LA TABLA COMP_VALES LA COMPROBACION DEL REEMBOLSO LIQUIDO DEL VALE REALIZADA POR FINANZAS ----------------------------*/
	public void insertaConceptoVale(Integer vale,double importe,double importeVale,int proyecto, String partida){
					
		int cons = (this.getJdbcTemplate().queryForInt("SELECT MAX(CONS_VALE)+1 FROM CONCEP_VALE WHERE CVE_VALE = ?", new Object[]{vale}));
	...
	public void verficaComprobacion(Integer vale){
		...
		//int CONS_VALE=getJdbcTemplate().queryForInt("select MAX(CONS_VALE) from COMP_VALES where CVE_VALE=?", new Object[]{vale}); //,Integer.class
		...
		Double importe = (Double)this.getJdbcTemplate().queryForObject("SELECT ISNULL(SUM(MV.IMPORTE)-(SELECT ISNULL(SUM(FV.IMPORTE),0.00) FROM SAM_FACTURAS_VALES FV " +
																	   "INNER JOIN SAM_FACTURAS F ON F.CVE_FACTURA=FV.CVE_FACTURA " +
																	   "WHERE F.STATUS IN (1,3) AND FV.CVE_VALE=MV.CVE_VALE),0.00)PORCOMPROMBAR FROM SAM_MOV_VALES MV " +
																	   "INNER JOIN SAM_VALES_EX V ON V.CVE_VALE=MV.CVE_VALE " +
																	   "WHERE V.STATUS=4 AND V.CVE_VALE=? " +
																	   "GROUP BY MV.CVE_VALE",new Object []{vale},Double.class); 
			
		/*
		if(importe<0) 
			 throw new RuntimeException("El importe es mayor al restante disponible en programa/partida, revise y vuelva a intentar esta operación "+vale+' '+ importe+' '+importeVale+ ' '+proyecto+' '+partida);
		*/
