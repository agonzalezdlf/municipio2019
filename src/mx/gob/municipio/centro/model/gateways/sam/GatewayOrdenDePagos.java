...
public Map getOrden(Long idOrden) {	   
	   return this.getJdbcTemplate().queryForMap("SELECT A.CVE_OP, A.EJERCICIO, A.NUM_OP, A.TIPO, CONVERT(varchar(10), A.FECHA, 103) AS FECHA, CONVERT(varchar(10), A.FECHA_CIERRE, 103) AS FECHA_CIERRE, A.FECHA_CIERRE AS FECHA_CIERRE2, A.FECHA_CIERRE AS FECHA_CIERRE2, A.FECHA AS V_FECHA, A.CLV_BENEFI, A.CVE_PERS, A.ID_DEPENDENCIA, " + 
					"CASE MONTH(A.FECHA) " +
					...
					"END AS PERIODO_TEXT, " +
					" CONVERT(varchar(10),A.FECHA_RECEP_P,103)RECEPCION_PROG,CONVERT(varchar(10),A.FECHA_RECEP_F,103)RECEPCION_FIN, "+
					...
					"C.CLV_FUENTE, C.CLV_RECURSO, (SELECT ISNULL(SUM(IMPORTE),0) AS IMPORTE_V FROM COMP_VALES WHERE COMP_VALES.CVE_OP = A.CVE_OP AND COMP_VALES.TIPO NOT IN ('CF','FA')) AS DESCUENTO_VALES," + 
               " A.ID_RECURSO, A.REEMBOLSOF, A.CONCURSO, ISNULL(A.IMPORTE,0) AS IMPORTE, A.RETENCION, A.IMP_NETO, A.NOTA, A.STATUS,  " +
...
public List getOrdenesTipo( String idUnidad, Integer cve_pers, Integer Ejercicio, Integer status) {
	   ...
	   return this.getJdbcTemplate().queryForList("SELECT A.CVE_OP, A.EJERCICIO, A.NUM_OP, A.TIPO, convert(varchar(10),A.FECHA,103) FECHA, A.CLV_BENEFI, A.CVE_PERS, A.ID_DEPENDENCIA, A.REEMBOLSOF, A.CONCURSO, A.IMPORTE,"+ 
                   " A.RETENCION, A.IMP_NETO, A.FE_PAGO, A.NOTA, A.STATUS,A.PERIODO,C.DESCRIPCION_ESTATUS, A.IVA, A.IMPORTE_IVA ,b.NCOMERCIA , D.DESCRIPCION AS TIPO_DOC  FROM SAM_ORD_PAGO A ,cat_benefi  B ,SAM_ESTATUS_OP C ,SAM_TIPO_ORDEN_PAGO D where a.clv_benefi=b.clv_benefi AND  A.STATUS=C.ID_ESTATUS  AND  A.TIPO=D.ID_TIPO_ORDEN_PAGO AND A.ID_DEPENDENCIA=? AND A.CVE_PERS = ? AND A.ejercicio=? and A.STATUS=? order by a.CVE_OP", new Object[]{idUnidad,cve_pers, Ejercicio,status});
}


public List<Map<String, Object>> getListaDeOrdenesPorEjemplo(String unidad, String  estatus , Date fInicial, Date fFinal ,String clv_benefi, Integer ejercicio, String tipoGasto, Integer idUsuario, String verUnidad, String tipo, String numop, boolean privilegio, String capitulo){
	...
	//Status 7 == Cancelado por EJERCIDO
	if(estatus.equals("7"))
	...
	else if(estatus.equals("6,7"))
		{
			...
			if(estatus.contains("7"))
				{
					estatus = estatus.replace('7', '6');
					sql += " AND a.STATUS in ("+estatus+")   AND (OP.FECHA_PAGO IS NOT NULL) ";
				}
			else			
...
/*Valida aqui si el documento esta en el periodo actual para permitir aperturar*/
            	...
            	for(Long cveOrden:cvOrdenes){
	                ...
	                List<Map<String, Object>> detalles = gatewayDetallesOrdenDePagos.getDetallesOrdenes(cveOrden);	                
...
public List<Map<String, Object>> getListadoOrdenesPagoDesejercer(int mes, int ejercicio){
	Map parametros =  new HashMap<String,Object>();
	/*Asignacion de parametros*/
	parametros.put("ejercicio", ejercicio);
	parametros.put("mes", mes);
...
public List<Map<String, Object>> getListadoOrdenesPagoenProgramacion(int mes,int moment, int ejercicio,String num_op, Date fechamovto){
	Map<String, Object> parametros =  new HashMap<String,Object>();
	/*Asignacion de parametros*/
	int listado=0;
	parametros.put("ejercicio", ejercicio);
	parametros.put("mes", mes);
	parametros.put("momento", moment);
	parametros.put("txtsearchop", num_op);
	parametros.put("txtfechanueva2", fechamovto);
	
	String sql = " SELECT OP.CVE_OP,OP.NUM_OP,OP.TIPO,OP.ID_RECURSO,OP.ID_DEPENDENCIA,B.NCOMERCIA,OP.NOTA,OP.IMPORTE,convert(varchar(10),OP.FECHA ,103)FECHA, convert(varchar(10),OP.FECHA_CIERRE,103)FECHA_CIERRE, OP.PERIODO,YEAR(OP.FECHA)ANIO,MONTH(OP.FECHA)MES,DAY(OP.FECHA)DIA,convert(varchar(10),OP.FECHA_RECEP_P,103)RECEPCION_PROG,convert(varchar(10),OP.FECHA_RECEP_F,103)RECEPCION_FIN  " +
				 " FROM SAM_ORD_PAGO OP "+ 
				 " INNER JOIN CAT_BENEFI B ON (B.CLV_BENEFI = OP.CLV_BENEFI) "+ 
				 " WHERE OP.STATUS=0 ";
	
	
	 if(moment==1) sql += " and (FECHA_RECEP_P IS NULL)"; //POR RECIBIR
	 if(moment==2) sql += " and (FECHA_RECEP_P IS NOT NULL)"; //POR DEVOLVER
	
	 if(mes!=0) sql += " and MONTH(OP.FECHA)=:mes";
	 
	 if(num_op!=null&&!num_op.equals("")){
			sql+= " AND OP.NUM_OP LIKE '%"+num_op+"%'";
		}
...
public List <Map<String, Object>> getMovimientosOPPresupuest(Long cve_op){
	List <Map<String, Object>> lista = this.getJdbcTemplate().queryForList("SELECT M.ID_PROYECTO, M.CLV_PARTID, SUM(M.MONTO) AS IMPORTE  FROM SAM_MOV_OP AS M "+
...
public List <Map<String, Object>> getMovimientosOP(Long cve_op){
...
public void ejercerOrdenPagoFinal(Long cve_op, Date fecha_ejerce, int ejercicio, int cve_pers) throws ParseException{
		...
		*/
		...
		if(existenFacturas(cve_op)){
			...
			List<Map<String, Object>> conceptos = this.gatewayDetallesOrdenDePagos.getDetallesOrdenes(cve_op);
			for(Map<String, Object> item : conceptos){
	...
	public boolean cancelarEjercidoOrdenPago(final Long cve_op, final String texto, final int ejercicio, final int cve_pers){
		try {   
			this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	            ...
	            protected void   doInTransactionWithoutResult(TransactionStatus status) {
	    			...
	    			List<Map<String, Object>> mov = getMovimientosOP(cve_op);
	    			//Comrpobamos si existen facturas en la Orden para devolver el Devengado
	    			if(existenFacturas(cve_op)){
	    				Date fecha_finalizado = new Date();
	    				List<Map<String, Object>> conceptos = gatewayDetallesOrdenDePagos.getDetallesOrdenes(cve_op);
	    				for(Map<String, Object> item : conceptos){
	    					if(item.get("CVE_FACTURA")!=null)
	    					{
		...
		public void desejercerOrdenPagoFinal(Long cve_op, String texto, int ejercicio, int cve_pers)
		{
			...
			Map<String, Object> data = this.getJdbcTemplate().queryForMap(SQL, new Object []{OrdenPago.get("CVE_OP").toString()});
			
			//mes del ejercido
			String month = months[Integer.parseInt(data.get("Mes").toString())-1].substring(0, 3);
			
			//Obtener lo movimientos de la OP
			List<Map<String, Object>> mov = this.getMovimientosOP(cve_op);
...
public boolean cambiarFechaOrdenPago(Long cve_op, String fechaNueva, int ejercicio, int cve_pers){
	...
	public void ingresoOPProgramacion(Long cve_op, String fecha, int ejercicio, int cve_pers){
		Date fechaActual = new Date();
		String folio=rellenarCeros(cve_op.toString(),6);
		this.getJdbcTemplate().update("UPDATE SAM_ORD_PAGO SET FECHA_RECEP_P=? WHERE CVE_OP=?", new Object[]{this.formatoFecha(fecha), folio});
		//this.formatoFecha
		gatewayBitacora.guardarBitacora(gatewayBitacora.OP_RECEP_EN_PROGRAMACION, ejercicio, cve_pers, cve_op, folio, "OP", fechaActual, null, null, "", Double.parseDouble("0".toString()));
	}
	
	//Validacion de Orden de Pago ingresada en la Dirección de Programación
	public void DevolucionOPPROG(Long cve_op, String fecha, int ejercicio, int cve_pers, String motivo){
		Date fechaActual = new Date();
		...
		String folio=rellenarCeros(cve_op.toString(),6);
		if (es_recibida>0){
			this.getJdbcTemplate().update("UPDATE SAM_ORD_PAGO SET FECHA_RECEP_P=? WHERE CVE_OP=? AND FECHA_RECEP_F IS NULL ", new Object[]{null,folio});
			gatewayBitacora.guardarBitacora(gatewayBitacora.OP_DEV_RECEP_EN_PROGRAMACION, ejercicio, cve_pers, cve_op, folio, "OP", fechaActual, null, null, motivo, Double.parseDouble("0".toString()));
		}else	
			throw new RuntimeException("La orden de Pago no puede ser devuelta por que no esta en programación");
	...
	public List<Map<String, Object>> cargarRelaciones(String tipo){
		List <Map<String, Object>> lst =this.getJdbcTemplate().queryForList("SELECT R.ID_RELACION, (R.FOLIO+' | '+U.LOGIN) AS DESCRIPCION, NUMERO, FOLIO, CONVERT(VARCHAR(10), FECHA, 103) AS FECHA, R.TIPO_RELACION, R.CVE_PERS, ID_GRUPO  FROM SAM_OP_RELACION R INNER JOIN SAM_USUARIOS_EX U ON (U.CVE_PERS = R.CVE_PERS) WHERE R.ACTIVO ='S' AND R.TIPO_RELACION =? ORDER BY R.NUMERO DESC", new Object[]{tipo});  
		return lst;
	}
	
	public List<Map<String, Object>> cargarRelacionesDocumentos(Long id_relacion){
		try{
				String sql = "";
	...
	public List<Map<String, Object>> cargarRelacionesGeneral(String tipo, String mes){
		String sql = "";
