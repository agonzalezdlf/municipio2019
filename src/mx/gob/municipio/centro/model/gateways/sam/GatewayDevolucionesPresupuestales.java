...
public class GatewayDevolucionesPresupuestales extends BaseGateway {
	...
	public List<Map<String, Object>> getDetallesDevolucion(Long idDevolucion){
	...
	public List<Map<String, Object>> getDetallesDevolucionReporte(Long idDevolucion){
	...
	public String cerrarDevolucion(final Long idDevolucion, final int ejercicio){
		...
		this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
            ...
            protected void   doInTransactionWithoutResult(TransactionStatus status) {
            	boolean cerrar = true;
            	Double ejercido = 0d;
            	Map<String, Object> dato = getJdbcTemplate().queryForMap("SELECT (SELECT ISNULL(SUM(DT.IMPORTE),0) FROM SAM_DEVOLUCION_PRESUPUESTAL_DETALLES AS DT WHERE DT.ID_DEVOLUCION = SAM_DEVOLUCION_PRESUPUESTAL.ID_DEVOLUCION) AS IMPORTE, PERIODO FROM SAM_DEVOLUCION_PRESUPUESTAL WHERE ID_DEVOLUCION = ?", new Object[]{idDevolucion});
            	...
            	Double importe = 0d;
            	List <Map<String, Object>> dt = getDetallesDevolucion(idDevolucion);
		...
		this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
            ...
            protected void   doInTransactionWithoutResult(TransactionStatus status) {
            	for(Long id : idDevoluciones){
            		boolean cerrar = true;
            		Double ejercido = 0d;
                	Map<String, Object> dato = getJdbcTemplate().queryForMap("SELECT STATUS, (SELECT ISNULL(SUM((DT.IMPORTE)),0) FROM SAM_DEVOLUCION_PRESUPUESTAL_DETALLES AS DT WHERE DT.ID_DEVOLUCION = SAM_DEVOLUCION_PRESUPUESTAL.ID_DEVOLUCION) AS IMPORTE, PERIODO FROM SAM_DEVOLUCION_PRESUPUESTAL WHERE ID_DEVOLUCION = ?", new Object[]{id});
                	int periodo = Integer.parseInt(dato.get("PERIODO").toString());
                	Double importe = 0d;
                	...
                	List <Map<String, Object>> dt = getDetallesDevolucion(id);
                	for(Map<String, Object> row : dt){
	...
	public String aperturarDevolucion(final Long[] idDevoluciones){
		...
		this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
            ...
            protected void   doInTransactionWithoutResult(TransactionStatus status) {
            	for(Long id : idDevoluciones){
            		Map<String, Object> dato = getJdbcTemplate().queryForMap("SELECT STATUS FROM SAM_DEVOLUCION_PRESUPUESTAL WHERE ID_DEVOLUCION = ?", new Object[]{id});
            		if(!dato.get("STATUS").toString().equals("1")) {
            			exito = false;
		...
		this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
            ...
            protected void   doInTransactionWithoutResult(TransactionStatus status) {
            	for(Long id : idMovtos){
            		Map<String, Object> movto = getJdbcTemplate().queryForMap("SELECT * FROM SAM_MOV_OP WHERE ID_MOV_OP = ?", new Object[]{id});
