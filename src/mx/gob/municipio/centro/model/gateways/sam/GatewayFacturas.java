...
public class GatewayFacturas extends BaseGateway {
	...
	public final static int FAC_STATUS_NUEVO = 0;
	public final static int FAC_STATUS_CERRADO = 1;
	public final static int FAC_STATUS_CANCELADO = 2;
	public final static int FAC_STATUS_FINIQUITADA = 3;
	
	public final static int FAC_TIPO_NOMINA = 1;
	public final static int FAC_TIPO_OBRAS = 2;
	public final static int FAC_TIPO_PEDIDO = 3;
	public final static int FAC_TIPO_OTOS = 4;
	public final static int FAC_TIPO_VALE = 5;
	public final static int FAC_TIPO_CONTRATO = 6;
	public final static int FAC_TIPO_HONORARIOS = 7;
	public final static int FAC_TIPO_ARRENDAMIENTOS = 8;
	public final static int FAC_TIPO_FONDO_FIJO = 9;
	public final static int FAC_TIPO_CON_RETENCION = 10;
	...
	private List<Map<String, Object>> presupuesto = new ArrayList<Map<String, Object>>();
	...
	public Map<String, Object> getPresupuesto(String tipo_doc, Long cve_doc, int ejercicio)
	{
	...
	public Long guardarFactura(Long cve_factura, String tipo_doc, Long cve_doc, int idTipoFactura, int idDependencia, int idProyecto, String clv_partid, String clv_benefi, int idEntrada, String num_fact, Double iva, Double subtotal, Double total,  String observacion, Date fecha_doc, int ejercicio, int cve_pers)
	{
		try
		{
			/*
			String tipoDocto = "";
			if(tipo_doc.equals("4")) tipoDocto = "CVE_REQ";
			if(tipo_doc.equals("3")) tipoDocto = "CVE_PED";
			if(tipo_doc.equals("6")) tipoDocto = "CVE_CONTRATO";*/
			
			if(cve_factura==null)
				cve_factura = 0L;
				
			if(cve_factura==0)//Si no existe la factura 
			{
				...
				if(tipo_doc.equals("5"))
				{
					...
					this.getJdbcTemplate().update("INSERT INTO SAM_FACTURAS("+tipo_doc+", NUM_FACTURA, ID_ENTRADA, CLV_BENEFI, CVE_PERS, ID_TIPO, ID_DEPENDENCIA, NOTAS, EJERCICIO, PERIODO, FECHA, FECHA_DOCUMENTO, SUBTOTAL, IVA, TOTAL, STATUS) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", new Object[]{
							cve_doc,
							num_fact,
							idEntrada,
							clv_benefi,
			...
			else//Si existe la factura...
			{
				if(tipo_doc.equals("5"))
				{
					...
					this.getJdbcTemplate().update("UPDATE SAM_FACTURAS SET "+tipo_doc+"=?, CLV_BENEFI=?, ID_TIPO=?, ID_DEPENDENCIA=?, ID_ENTRADA=?, NOTAS=?, FECHA_DOCUMENTO=?, SUBTOTAL=?, IVA=?, TOTAL=? WHERE CVE_FACTURA =? ", new Object[]{
						cve_doc,
						clv_benefi,
						idTipoFactura,
						idDependencia,
...
//-------------------------------Validar si se finiquita el PEDIDO al cerrar el devengado......................12/05/17
	...
	public List<Map<String, Object>> getListadoFacturas2(String num_factura, String unidad, String estatus, Date fInicial, Date fFinal ,String cve_benefi, String tipo_gto, Integer ejercicio, Integer idUsuario,
			String verUnidad, String cve_uniusr, String numped, String numreq, boolean privilegio) {
		
		Map<String, Object> parametros =  new HashMap<String,Object>();
		
		/*Asignacion de parametros*/
		parametros.put("idUnidad", unidad);
		...
		String sql = "SELECT DISTINCT F.CVE_FACTURA " +
			  ...
			  "FROM SAM_FACTURAS AS F " +
			  		"LEFT  JOIN SAM_FACTURA_DETALLE FD ON FD.CVE_FACTURA=F.CVE_FACTURA " +
			  		"LEFT JOIN CAT_DEPENDENCIAS AS DEP ON (DEP.ID = F.ID_DEPENDENCIA) " +
					"LEFT JOIN SAM_REQUISIC AS R ON (R.CVE_REQ = F.CVE_REQ) " +
					"LEFT JOIN  CEDULA_TEC AS B ON (B.ID_PROYECTO = FD.ID_PROYECTO) " + 
					"LEFT JOIN SAM_PEDIDOS_EX AS P ON (P.CVE_PED = F.CVE_PED) " +
					"LEFT JOIN SAM_CONTRATOS AS CO ON (CO.CVE_CONTRATO = F.CVE_CONTRATO) " +
		...
		if (verUnidad==null&&!privilegio)
			sql+= " AND (F.CVE_PERS=:cve_pers) ";
		
		if(verUnidad!=null&&!privilegio)
			sql+= " AND (F.CVE_PERS=:cve_pers OR F.ID_DEPENDENCIA=:idUnidad) ";
		
		if (verUnidad==null&&privilegio&&unidad.equals("0"))
			sql+= " ";
		if (verUnidad==null&&privilegio&&!unidad.equals("0"))
			sql+= " AND  F.ID_DEPENDENCIA=:idUnidad ";
		if(verUnidad!=null&&privilegio)
			sql+= " AND (F.CVE_PERS=:cve_pers OR F.ID_DEPENDENCIA=:idUnidad) ";
		
		if(cve_benefi!=null&&!cve_benefi.equals("")) 
			sql+= " AND F.CLV_BENEFI=:cve_benefi ";
		
		if(numped!=null&&!numped.equals(""))
			sql+= " AND P.NUM_PED LIKE '%"+numped+"%'";
		
		if(numreq!=null&&!numreq.equals(""))
			sql+= " AND R.NUM_REQ LIKE '%"+numreq+"%'";
		
		return this.getNamedJdbcTemplate().queryForList(sql+" ORDER BY F.CVE_FACTURA", parametros);
	}
	...
	public List<Map> getListadoFacturas(Map m){
		...
		if(m.get("clv_benefi")!=null&&!m.get("cve_benefi").equals(""))
...
//--------------------------------------------------------------------------------
	...
	public Map getFactura(Long cve_factura){
		try
		{
			String sql = "SELECT F.CVE_FACTURA " +
				  ...
				  "FROM SAM_FACTURAS AS F " +
						"LEFT JOIN SAM_REQUISIC AS R ON (R.CVE_REQ = F.CVE_REQ) " +
						"LEFT JOIN SAM_CAT_TIPO_FACTURAS CTF ON (CTF.Id_TipoFactura=F.ID_TIPO) " +
						"LEFT JOIN SAM_PEDIDOS_EX AS P ON (P.CVE_PED = F.CVE_PED) " +
						"LEFT JOIN SAM_VALES_EX AS V ON (V.CVE_VALE = F.CVE_VALE) "+
						"LEFT JOIN SAM_CONTRATOS AS CON ON (CON.CVE_CONTRATO = F.CVE_CONTRATO) "+
	...
	private boolean tieneVale(Long idFactura){
		return this.getJdbcTemplate().queryForInt("SELECT COUNT(*) AS N FROM SAM_FACTURAS_VALES FV INNER JOIN COMP_VALES CV ON CV.ID_MOVAL_FACTURA=FV.ID_MOVIMIENTO WHERE FV.CVE_FACTURA=?", new Object[]{idFactura})>0;
	}
		 ...
		 this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	            ...
	            protected void   doInTransactionWithoutResult(TransactionStatus status) {
	            	for(Long id : idFacturas)
	            	{
	            		...
	            		if (tieneVale(id));
	            		{
	            			System.out.println("La factura cuenta con vales");
							@SuppressWarnings("unchecked")
							List <Map<String, Object>> detallesVales = getJdbcTemplate().queryForList("SELECT * FROM SAM_FACTURAS_VALES FV INNER JOIN COMP_VALES CV ON CV.ID_MOVAL_FACTURA=FV.ID_MOVIMIENTO WHERE FV.CVE_FACTURA=? ", new Object[]{id});
		            		
		            		for(Map<String, Object> row: detallesVales){
		            			
		            			getJdbcTemplate().update("DELETE FROM COMP_VALES where ID_MOVAL_FACTURA=?  ", new Object[]{row.get("ID_MOVAL_FACTURA")});
		            			
		            			int existeconcep =getJdbcTemplate().queryForInt("SELECT COUNT(*) FROM CONCEP_VALE WHERE CVE_VALE=? AND CONS_VALE=?", new Object[]{row.get("CVE_VALE"),row.get("CONS_VALE")});
		            			
		            			if (existeconcep>0)
		            				getJdbcTemplate().update("DELETE CONCEP_VALE WHERE CVE_VALE=? AND CONS_VALE=?  ", new Object[]{row.get("CVE_VALE"),row.get("CONS_VALE")});
		            		}
	            		}
	            		System.out.println("La factura no cuenta con vales");
	...
	public void eliminarArchivoFactura(int cve_pers, Long idArchivo, HttpServletRequest request){
		...
		Map<String, Object> archivo = this.getJdbcTemplate().queryForMap("SELECT *FROM SAM_FACTURAS_ARCHIVOS WHERE ID_ARCHIVO =?", new Object[]{idArchivo});
		/*eliminado fisico*/
	...
	public List<Map<String, Object>> getListaFacturasOrdenPago(int idRecurso, int idDependencia, String clv_benefi, int cve_pers){
	...
	public List<Map<String, Object>> getListaAnexosArchivosFacturas(Long cve_op){
	...
	private boolean getDisponiblePresupuesto(int idProyecto, String clv_partid, Double monto){
		for(Map<String, Object> row: presupuesto){
	...
	private List<Map<String, Object>> GetPayRollByGroup()
	{
		try
		{
			List<Map<String, Object>> nomina = getJdbcTemplate().queryForList(
	...
	private List<Map<String, Object>> GetDeductives(String TipoNomina, int IdRecurso, String Clv_UniAdm)
	{
		try
		{
	...
	private Double SaveDeductivesAndRetention(List<Map<String, Object>> Deductives, Long cve_factura)
	{
		try
		{
			int c = 0;
			...
			for(Map<String, Object> rw:Deductives){
				c++;
	...
	private void CreatePayOrder(Long cve_factura, int idDependencia, int idRecursoAnt, int idGrupoFirma, int periodoAnt, String fecha_nomina, Double retencAnt, String Nota, int ejercicio, int cve_pers)
	{
		try
		{
			...
			List<Map<String, Object>> facturas = getJdbcTemplate().queryForList("SELECT F.*, FAC.NUM_FACTURA FROM SAM_FACTURA_DETALLE AS F INNER JOIN SAM_FACTURAS AS FAC ON (FAC.CVE_FACTURA = F.CVE_FACTURA) WHERE F.CVE_FACTURA IN ("+cve_factura+") ORDER BY F.CVE_FACTURA ASC");
			for(Map<String, Object> rw : facturas)
			{
			...
			int i =0;
			List<Map<String, Object>> retenciones = getJdbcTemplate().queryForList("SELECT *FROM SAM_FACTURA_MOV_RETENC WHERE CVE_FACTURA =?", new Object[]{cve_facturaTemp});
			for(Map<String, Object> retenc : retenciones)
			{
				i++;
		...
		try
		{
			this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
                ...
                protected void doInTransactionWithoutResult(TransactionStatus status) {
            		...
            		String Note = "";//
                	int idGrupoFirma = getJdbcTemplate().queryForInt("SELECT ID_GRUPO_CONFIG FROM SAM_GRUPO_CONFIG_USUARIO WHERE ASIGNADO =1 AND ID_USUARIO=? ", new Object[]{cve_pers}); //(19) Grupo de firmas Direccion de Administracion
                	
                	//Agrupado de Nomina por TIPO_NOMINA, CLV_UNIADM Y ID_RECURSO
                	List<Map<String, Object>> nomina = GetPayRollByGroup();
                	for(Map row: nomina)
                	{
                		valor++;
                		...
                		if(!tipoNomina.equals(row.get("TIPO_NOMINA").toString()) && idRecurso != ((Integer) row.get("ID_RECURSO")) && !clv_uniadm.equals(row.get("CLV_UNIADM").toString())){
            				...
            				List<Map<String, Object>> Deductives = GetDeductives(row.get("TIPO_NOMINA").toString(), (Integer)row.get("ID_RECURSO"), row.get("CLV_UNIADM").toString());
			  ...
			  this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	                @Override
	                	...
	                	int idGrupoFirma=436; //(19) Grupo de firmas Direccion de Administracion
	                		...
	                		for(Map<String, Object> row: nomina)
	                		{
	                			//Valida aqui el preupuesto del proyecto y partida para cerrar las facturas
	                			boolean disponible = getDisponiblePresupuesto((Integer) row.get("ID_PROYECTO"), row.get("CLV_PARTID").toString(), Double.parseDouble(row.get("IMPORTE").toString()));
	                			...
	                			List<Map<String, Object>> deductivas = getJdbcTemplate().queryForList("SELECT  A.TIPO_NOM, A.ID_RECURSO, A.RECINTO, CU.UNIDADADM, A.CLV_RETENC, SUM (A.TOTAL) AS IMPORTE FROM SAM_NOMINA_DEDUCCIONES AS A "+
																									"INNER JOIN CAT_UNIADM AS CU ON (CU.CLV_UNIADM = A.RECINTO) " +
																								"WHERE A.TIPO_NOM =? AND A.ID_RECURSO =? AND A.RECINTO =? "+
																								"GROUP BY A.TIPO_NOM, A.ID_RECURSO, A.RECINTO, CU.UNIDADADM, A.CLV_RETENC "+
	                			...
	                			if(tipoNomina.equals(row.get("TIPO_NOMINA").toString())&&clv_uniadm.equals(row.get("CLV_UNIADM").toString())&&idRecurso==((Integer) row.get("ID_RECURSO"))){
	                				...
	                				for(Map<String, Object> rw:deductivas){
	                					c++;
	                					retencAnt += Double.parseDouble(rw.get("IMPORTE").toString());
	                					getJdbcTemplate().update("INSERT INTO SAM_FACTURA_MOV_RETENC(CVE_FACTURA, CONS, CLV_RETENC, IMPORTE) VALUES (?,?,?,?)", new Object[]{
	                							cve_factura,
	                				...
	                				if(total_req==valor){
					                			...
					                			//Guarda los movimientos de la orden de Pago
					                			List<Map<String, Object>> facturas = getJdbcTemplate().queryForList("SELECT F.*, FAC.NUM_FACTURA FROM SAM_FACTURA_DETALLE AS F INNER JOIN SAM_FACTURAS AS FAC ON (FAC.CVE_FACTURA = F.CVE_FACTURA) WHERE F.CVE_FACTURA IN ("+temp+") ORDER BY F.CVE_FACTURA ASC");
					                			for(Map<String, Object> rw : facturas)
					                			{
					                				getJdbcTemplate().update("INSERT INTO SAM_MOV_OP(CVE_OP, CVE_FACTURA, ID_PROYECTO, CLV_PARTID, NOTA, TIPO, MONTO) VALUES(?,?,?,?,?,?,?)", new Object[]{
					                						cve_op,
...
//------------------------------------------ Revisar para escribir el comp_vales desde la Facturas -------------------------------------------------//
	...
	public Map<String, Object> getFacturaVALES(final Integer ID_MOVIMIENTO){
		try
		{
	            ...
	            this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	                ...
	                protected void   doInTransactionWithoutResult(TransactionStatus status) {	 
	                	...
	                	for (Integer idVale :lstVales) {	
	                		...
	                		contconcep_vale =(int) getJdbcTemplate().queryForObject("SELECT COUNT(COV.CONS_VALE) FROM CONCEP_VALE COV LEFT JOIN COMP_VALES CV ON CV.CVE_VALE=COV.CVE_VALE AND CV.CONS_VALE=CV.CONS_VALE WHERE CV.ID_MOVAL_FACTURA=? AND CV.CONS_VALE=COV.CONS_VALE",new Object[]{Integer.parseInt((idVale).toString())},Integer.class);
	...
	public List<Map<String, Object>> getDetallesProyectoPartidaDocumento(Long cve_factura, int ejercicio)
	{
		Map<String, Object> factura = this.getFactura(cve_factura);
		int mesActivo = gatewayMeses.getMesActivo(ejercicio);
