...
public class GatewayContratos extends BaseGateway {
	...
	public Map<String, Object> getContrato(Long cve_contrato){
		return this.getJdbcTemplate().queryForMap("SELECT C.CVE_CONTRATO, " +
														" C.NUM_CONTRATO, " +
														" C.CVE_DOC, " +
														" C.EJERCICIO, " +
														"CASE C.ID_TIPO " +
														...
														"CONVERT(varchar(10), C.FECHA_INICIO, 103) AS FECHA_INICIO, " +
														 ...
														 "CONVERT(varchar(10), C.FECHA_CERRADO, 103) AS FECHA_CERRADO, " +
														 "CONVERT(varchar(10), C.FECHA_CERRADO, 103) AS FECHA_CIERRE, " +
														 "C.FECHA_CERRADO AS FECHA_CERRADO2, "+
														 "B.NCOMERCIA AS PROVEEDOR, " +
														 "R.RECURSO, " +
														 "D.CLV_UNIADM,"+
														 "D.DEPENDENCIA, " +
														 "TC.DESCRIPCION, " +
														 "(SELECT ISNULL(SUM(M.IMPORTE),0) FROM SAM_COMP_CONTRATO AS M WHERE M.CVE_CONTRATO = C.CVE_CONTRATO) AS IMPORTE "+
												"FROM SAM_CONTRATOS AS C " +
													"LEFT JOIN SAM_CAT_TIPO_CONTRATOS AS TC ON (TC.ID_TIPO=C.ID_TIPO) " +
													"INNER JOIN CAT_BENEFI  AS B ON (B.CLV_BENEFI = C.CLV_BENEFI) " +
													"INNER JOIN CAT_RECURSO AS R ON (R.ID = C.ID_RECURSO) " +
													"INNER JOIN CAT_DEPENDENCIAS AS D ON (D.ID = C.ID_DEPENDENCIA) " +
													"LEFT JOIN SAM_REQUISIC AS REQ ON (REQ.CVE_REQ = C.CVE_DOC) " +
													"LEFT JOIN SAM_PEDIDOS_EX AS PED ON (PED.CVE_PED = C.CVE_DOC) " +
												"WHERE C.CVE_CONTRATO = ?",new Object[]{cve_contrato});
	}
	...
	public Long guardarContrato(Long cve_contrato, int idDependencia, String num_contrato, String fecha_ini, String fecha_fin, String oficio, String tiempo_entrega, int tipo, String concepto, Double anticipo, int idRecurso, String clv_benefi, Long cve_doc, int ejercicio, int cve_pers, int idGrupo){
		...
		if(cve_contrato==0){
			...
			Map<String, Object> Contrato = this.getContrato(cve_contrato);
		...
		try {                 
	            this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	                ...
	                protected void   doInTransactionWithoutResult(TransactionStatus status) {
	                	...
	                	for(Map<String, Object> row: conceptos){
		...
		try {                 
            this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
                ...
                protected void   doInTransactionWithoutResult(TransactionStatus status) {
                		...
                		Map<String, Object> contrato = getContrato(cve_contrato);//518
                		
                		List<Map<String, Object>> conceptos = getConceptosContratoAgrupados(cve_contrato);
                		...
                		if(contrato.get("CVE_DOC")!=null)//Cliclo donde se contrata atravez del presupuesto de otro documento
                		{
                			...
                			Map<String, Object> requisicion = gatewayRequisicion.getRequisicion(Long.parseLong(contrato.get("CVE_DOC").toString()));
            ...
            this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
                ...
                protected void   doInTransactionWithoutResult(TransactionStatus status) {
                	for(Long cve_contrato: lst_contratos){
                		...
                		Map<String, Object> documento = getContrato(cve_contrato);
	...
	public List <Map<String, Object>> getListaContratos(Long cve_contrato, String num_contrato, int idDependencia, int mesActivo, int idRecurso){
		...
		return this.getJdbcTemplate().queryForList("SELECT  B.CVE_CONTRATO, "+
																...
																"WHERE B.STATUS IN (1) AND A.PERIODO = " + mesActivo + " "
	...
	public List<Map<String, Object>> getConceptosContratoAgrupados(Long cve_contrato){
	...
	public List<Map<String, Object>> getArchivosContrato(Long cve_contrato){
	...
	public void eliminarArchivoContrato(int cve_pers, Long idArchivo, HttpServletRequest request){
		...
		Map<String, Object> archivo = this.getJdbcTemplate().queryForMap("SELECT *FROM SAM_CONTRATOS_ARCHIVOS WHERE ID_ARCHIVO =?", new Object[]{idArchivo});
		/*eliminado fisico*/
	...
	public List<Map<String, Object>> getListaValesPresupuesto(Long cve_vale, String clv_benefi, int idRecurso, int tipo_doc, int cve_pers, int idDependencia){
		String clausula = "";
	...
	// DEMO DEL LISTADO DE VALES PARA COMPROBAR DESDE CONTRATOS------------------------------------28-06-17
		...
		public List<Map<String, Object>> getListaValesContratos(String num_vales, int cve_pers, String cve_unidad, String clv_benefi){
