...
/********************    Metodo que genera el listado de requisiciones y busca mendiante los parametros *********************************************/
	...
	public Map getRequisicion(Long cve_req){
		return this.getJdbcTemplate().queryForMap("SELECT DISTINCT "+
															...
															"CAT_PARTID.PARTIDA,"+  
															"LEFT(CAT_RECURSO.RECURSO, 50) AS TIPO_GASTO,"+
                ...
                protected void   doInTransactionWithoutResult(TransactionStatus status) {
					...
					if(presupuest){
						...
						if(tipo_req==7 || tipo_req==1 ){
							...
							List <Map<String,Object>> mov = gatewayMovimientosRequisicion.getConceptos(cve_req);
							for(Map row: mov){
...
/*------------------------------------------Metodo privado que apertura la requisicion-----------------------------------------------------------------*/
	...
	public List <Map<String, Object>> getLightRequisicion(Long cve_req){
	...
	//Cambiar el estatus de la requisicion al elaborar un pedido 
	public boolean cambiaStatusRequisicion(Long cve_req){
	...
	public void cambiarStatusRequisicion(Long cve_req){
		...
		for(Map<String, Object> row: lotes){
	...
	private int tieneMasPedidos(Long cve_req){
		return this.getJdbcTemplate().queryForInt("SELECT COUNT(*) AS N FROM SAM_PEDIDOS_EX WHERE STATUS IN(0,1) AND CVE_REQ = ?", new Object[]{cve_req});
	}
	
	
public double getPreCompromisoTotal (Long idRequisicion ){
...
public double getPreCompromisoMensual (Long idRequisicion,int mes ){
	...
	public String enviarLotesPedido(Long id_movto, Long cve_ped, int ejercicio, int cve_pers){
		try{
			...
			Map<String, Object> lotes = gatewayMovimientosRequisicion.getConcepto(id_movto);
			Map<String, Object> req = this.getJdbcTemplate().queryForMap("SELECT NUM_REQ, FECHA, ID_PROYECTO, CLV_PARTID FROM SAM_REQUISIC WHERE CVE_REQ = ?", new Object[]{lotes.get("CVE_REQ").toString()});
	...
	public Map<String, Object> getImporteDisponibleRequisicion(Long cve_req){
	...
	public Map<String, Object> getFechaPeriodoRequisicion(Long cve_req){
	...
	public Map<String, Object> getBeneficiario(Long cve_doc){
		try {   
	            ...
	            protected void   doInTransactionWithoutResult(TransactionStatus status) {
		            	...
		            	Map<String, Object> req = getJdbcTemplate().queryForMap("SELECT NUM_REQ, CONVERT(varchar(10),FECHA,103) AS FECHA, ID_PROYECTO, CLV_PARTID, (SELECT ISNULL(SUM(CANTIDAD*PRECIO_EST),0) FROM SAM_REQ_MOVTOS WHERE SAM_REQ_MOVTOS.CVE_REQ = SAM_REQUISIC.CVE_REQ ) AS TOTAL, OT.CLV_BENEFI, C.NCOMERCIA, (SELECT COUNT(*) AS N FROM SAM_ORDEN_TRAB O WHERE O.CVE_REQ = SAM_REQUISIC.CVE_REQ) AS CONT "+  
																	"FROM SAM_REQUISIC "+ 
																		...
																		"WHERE SAM_REQUISIC.CVE_REQ = ?", new Object[]{cve_req});
		            	Map<String, Object> ben = getJdbcTemplate().queryForMap("SELECT NCOMERCIA FROM CAT_BENEFI WHERE CLV_BENEFI =?", new Object[]{clv_benefi});
	...
	public boolean reenumerarLotesDesde(final Long cve_req, final int num, final int cve_pers, final int ejercicio){
		try {   
					...
					this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
			            ...
			            protected void   doInTransactionWithoutResult(TransactionStatus status) {
				            	int n = num;
				            	List <Map<String,Object>> lotes = gatewayMovimientosRequisicion.getConceptos(cve_req);
				            	...
				            	Map<String, Object> req = getJdbcTemplate().queryForMap("SELECT NUM_REQ, CONVERT(varchar(10),FECHA,103) AS FECHA, ID_PROYECTO, CLV_PARTID, (SELECT ISNULL(SUM(CANTIDAD*PRECIO_EST),0) FROM SAM_REQ_MOVTOS WHERE SAM_REQ_MOVTOS.CVE_REQ = SAM_REQUISIC.CVE_REQ) AS TOTAL FROM SAM_REQUISIC WHERE CVE_REQ = ?", new Object[]{cve_req});
				            	/*Graba en bitacora el cambio*/
	...
	public Map<String, Object> getReembolsoRequisiciones(Long cve_req){
	...
	public void guardarReembolsoRequisicion(Long cve_ped, Double monto){
		if(monto==0){
			Map<String, Object> m = this.getReembolsoRequisiciones(cve_ped);
	...
	public BigDecimal getCompromisoFuturo(Long cve_req, int periodoActual)
	{
		...
		List<Map<String, Object>> lista = this.getJdbcTemplate().queryForList("SELECT VT_COMPROMISOS.* from VT_COMPROMISOS INNER JOIN SAM_REQUISIC ON (SAM_REQUISIC.CVE_REQ = VT_COMPROMISOS.CVE_DOC) WHERE SAM_REQUISIC.TIPO = 8 and VT_COMPROMISOS.TIPO_DOC in ('O.S') AND VT_COMPROMISOS.CVE_DOC =? AND VT_COMPROMISOS.MONTO>0 AND VT_COMPROMISOS.PERIODO>?", new Object[]{cve_req, periodoActual});
