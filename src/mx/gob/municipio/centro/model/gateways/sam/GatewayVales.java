...
public class GatewayVales  extends BaseGateway {
	...
	public Long guardar( Date fecha,String  tipoVale,String claveBeneficiario,String justificacion,/*String proyecto, String partida,*/
		...
		Map<String, Object> vale = this.getJdbcTemplate().queryForMap("SELECT *FROM SAM_VALES_EX WHERE CVE_VALE = ? ", new Object[]{clave});
	...
	public Map<String, Object> getValeTodosDatos(Long  clave ){
	...
	public List<Map<String,Object>> getListaDeValesPorEjemplo(String unidad, String  estatus , String clv_benefi, String numvale, String numcontrato, Date fInicial, Date fFinal , Integer ejercicio, String tipoGasto, Integer idUsuario, String verUnidad, Boolean privilegio){
		...
		String sql = "SELECT     A.NUM_VALE, A.CVE_VALE, A.EJERCICIO, A.ID_DEPENDENCIA, A.CVE_PERS, A.CLV_BENEFI, A.ID_RECURSO, CR.RECURSO, "+ 
					 " CONVERT(varchar(10), A.FECHA, 103) AS FECHA, A.TIPO, A.MES, (SELECT ISNULL(SUM(IMPORTE),0) FROM  SAM_MOV_VALES WHERE SAM_MOV_VALES.CVE_VALE =A.CVE_VALE) AS TOTAL, A.JUSTIF, A.DOCTO_COMP, A.FECHA_INI, A.FECHA_FIN, A.FECHA_MAX, "+ 
                      ...
                      " E.TIPO_VALE, CO.NUM_CONTRATO "+
                      " FROM  SAM_VALES_EX AS A LEFT OUTER JOIN "+
                      " SAM_CONTRATOS CO ON CO.CVE_CONTRATO=A.CVE_CONTRATO INNER JOIN "+
                      " CAT_BENEFI AS B ON A.CLV_BENEFI = B.CLV_BENEFI INNER JOIN "+
                      " CAT_DEPENDENCIAS AS C ON A.ID_DEPENDENCIA = C.ID INNER JOIN "+
		...
		if(numvale!=null&&!numvale.equals("")){
			sql+= " AND A.NUM_VALE LIKE '%"+numvale+"%'";
		}
		
		if(numcontrato!=null&&!numcontrato.equals("")){
			sql+= " AND CO.NUM_CONTRATO LIKE '%"+numcontrato+"%'";
		}
	...
	public void actualizarEstatusVale( Long clave, Integer idStatus, Integer ejercicio, Integer cve_pers){
	  ...
	  public List<Map<String, Object>> getValesPagadosNoComprobados(String unidad, int ejercicio ){
		...
		try {   
			this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	            ...
	            protected void   doInTransactionWithoutResult(TransactionStatus status) {
	            	...
	            	Map<String, Object> val = getJdbcTemplate().queryForMap("SELECT NUM_VALE, CONVERT(varchar(10),FECHA,103) AS FECHA, IMPORTE, ID_PROYECTO, CLV_PARTID FROM SAM_VALES_EX WHERE CVE_VALE = ?", new Object[]{cve_vale});
			...
			this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	            ...
	            protected void   doInTransactionWithoutResult(TransactionStatus status) {
	            	for(Long id: cve_val){
						...
						Map<String, Object> Val = getJdbcTemplate().queryForMap("SELECT NUM_VALE, CONVERT(varchar(10),FECHA,103) AS FECHA, IMPORTE, ID_PROYECTO, CLV_PARTID FROM SAM_VALES_EX WHERE CVE_VALE = ?", new Object[]{id});
	            		@SuppressWarnings("unchecked")
		...
		try {   
			this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	            ...
	            protected void   doInTransactionWithoutResult(TransactionStatus status) {
		            	...
		            	Map<String, Object> val = getJdbcTemplate().queryForMap("SELECT NUM_VALE, CONVERT(varchar(10),FECHA,103) AS FECHA, IMPORTE, ID_PROYECTO, CLV_PARTID, SAM_VALES_EX.CLV_BENEFI, C.NCOMERCIA "+  
																	"FROM SAM_VALES_EX "+  
																		"LEFT JOIN CAT_BENEFI AS C ON (C.CLV_BENEFI = SAM_VALES_EX.CLV_BENEFI) "+
																		"WHERE SAM_VALES_EX.CVE_VALE= ?", new Object[]{cve_doc});
		            	Map<String, Object> ben = getJdbcTemplate().queryForMap("SELECT NCOMERCIA FROM CAT_BENEFI WHERE CAT_BENEFI.CLV_BENEFI =?", new Object[]{clv_benefi});
	            		String texto ="";
	...
	public void eliminarArchivoVale(int cve_pers, Long idArchivo, HttpServletRequest request){
		...
		Map<String, Object> archivo = this.getJdbcTemplate().queryForMap("SELECT *FROM SAM_VALES_ARCHIVOS WHERE ID_ARCHIVO =?", new Object[]{idArchivo});
		/*eliminado fisico*/
