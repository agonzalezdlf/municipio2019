		...
		try
		{
			...
			this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
				...
				if(id_salida==0)
				{
					...
					List <Map<String, Object>> detalles_entrada = gatewayEntradasDocumentos.getConceptos(id_entrada);
					for(Map<String, Object> row: detalles_entrada){
					...
					if(tipo_salida==1){
						...
						tipoIva2 = Integer.parseInt(salida.get("TIPO_IVA").toString());
						 ...
						 if(getJdbcTemplate().queryForInt("SELECT COUNT(*) FROM DETALLE_SALIDA WHERE ID_SALIDA =?", new Object[]{id_salida})==0){
							 List <Map<String, Object>> detalles_entrada = gatewayEntradasDocumentos.getConceptos(id_salida);
							 for(Map<String, Object> row: detalles_entrada){	
									getJdbcTemplate().update("INSERT INTO DETALLE_SALIDA(ID_SALIDA, ID_DETALLE_ENTRADA, ID_INVENTARIO, CANTIDAD, PRECIO, STATUS) VALUES(?,?,?,?,?,?)", new Object[]{
											id_salida,
											row.get("ID_DETALLE_ENTRADA"),
...
//------------------------------------------------CANCELAR SALIDAS--------------------------
	...
	public List<Map<String, Object>> gatewaySalidas(Long idSalida){
