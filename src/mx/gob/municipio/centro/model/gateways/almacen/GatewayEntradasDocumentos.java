...
public class GatewayEntradasDocumentos extends BaseGatewayAlmacen {
	...
	private void guardarDetalleDocumento(Long id_entrada, Long id_pedido){
		...
		List <Map<String, Object>> resultado = this.gatewayPedidos.getConceptos(id_pedido);
	...
	public List <Map<String, Object>> getConceptos(long id_entrada){
	...
	public String cerrarEntradaDocumento(final Long id_entrada){
		...
		this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
            ...
            protected void   doInTransactionWithoutResult(TransactionStatus status) {   
				...
				Map documento = getEntradaDocumento(id_entrada);
            	@SuppressWarnings("rawtypes")
				List <Map<String, Object>> result = getConceptos(id_entrada);
            	@SuppressWarnings("rawtypes")
		...
		try{
			 this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	                ...
	                protected void   doInTransactionWithoutResult(TransactionStatus status) {	
						...
						List <Map<String, Object>> conceptos = getConceptos(id_entrada);
						for(Map<String, Object> detalle: conceptos){
							Map<String, Object> detalle2 = getMapArticulo(Long.parseLong(entrada.get("ID_DEPENDENCIA").toString()), Long.parseLong(entrada.get("ID_ALMACEN").toString()), Long.parseLong(detalle.get("ID_ARTICULO").toString()));
	                ...
	                protected void   doInTransactionWithoutResult(TransactionStatus status) {	
						...
						for(Long idEntrada: id_entradas){
								if(getJdbcTemplate().queryForInt("SELECT COUNT(*) FROM SALIDAS WHERE ID_ENTRADA =? AND STATUS=1", new Object[]{idEntrada})>0)
									...
									List <Map<String, Object>> conceptos = getConceptos(idEntrada);
